...
1. **Primary — Tool-Based URL Context.** The API call includes `codeExecution` and `urlContext` tools, with the YouTube URL embedded in the prompt text. This lets Gemini fetch and analyze the video content directly.

2. **Failover — Native `fileUri` Input.** If the primary approach fails, we fall back to the official [Gemini Video Understanding API](https://ai.google.dev/gemini-api/docs/video-understanding#youtube) using the `fileData`/`fileUri` method with the video URL.
